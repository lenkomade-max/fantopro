...
export class Whisper {
  ...
  /**
   * Create captions with chunk processing for long audio files
   * This helps avoid memory issues with large audio files
   */
  async CreateCaptionWithChunks(
    audioPath: string,
    chunkDurationSec: number = 60,
  ): Promise<Caption[]> {
    // For now, just use the regular method
    // TODO: Implement actual chunk processing with FFmpeg audio splitting
    logger.debug({ audioPath, chunkDurationSec }, "Creating captions (chunk mode available but using regular mode)");
    return this.CreateCaption(audioPath);
  }
}
